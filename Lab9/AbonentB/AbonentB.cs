    ...
    class Skrzynka : IConsumer<IPubl>, IConsumer<Fault<IOdpA>>, IConsumer<Fault<IOdpB>>
    {
        public Task Consume(ConsumeContext<IPubl> context)
        {
            return Task.Run(() =>
            {
                if (context.Message.Numer % 3 == 0)
                    context.RespondAsync(new OdpB() { Kto = "abonent B" });

                Console.WriteLine($"Otrzymana wiadomosc: {context.Message.Numer} {(context.Message.Numer % 3 == 0 ? "wyslano odpowiedz" : "")}");
            });
        }
        ...
        public Task Consume(ConsumeContext<Fault<IOdpA>> context)
        {
            return Task.Run(() =>
            {
                ...
                {
                    Console.WriteLine($"\nWywalilo wyjatek !!!! {e.Message} {context.Message.Message.Kto}\n");
                }
            });
        }
